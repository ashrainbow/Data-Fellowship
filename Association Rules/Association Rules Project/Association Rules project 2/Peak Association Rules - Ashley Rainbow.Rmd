---
title: "Peak Association Rules Project"
output: html_notebook
---

# Introduction
Looking at sales from the peak period I want to see what products were sold and how going forward more products can be sold 
...
```{r}
setwd("C:/Users/Ashley Rainbow/Desktop")
```

# 3.Import the data
```{r}
orders <- read.csv('Dixons Peak Association Rules.csv')
```
...
```{r}
plot(cream_toaster_rule, method = 'graph', control = list(cex = 0.5))
```
...
# 24.Comparing the Cream Toaster rule results to the other rules.
...
# Summary for Cream Toaster
As you can see above, the cream toaster is commonly bought with products of the same colour from the same brand. This suggests that selling these products as a bundle, for example a cream kitchen combination set, which would include a cream toaster a cream kettle and a cream bread bin could result in an increase in sales for products of this group. 

> {Retro 4-Slice Toaster, Retro Jug Kettle, Retro Towel Pole, Retro Bread Bin Cream, Retro Microwave with Grill, Retro Tradiitional Kettle, Retro 45-litre Rectangular Bin, Solo Microwave, Retro 1-litre Canisters, Retro 2-piece Non-stick Frying Pan Set}
